...
#  See ESP32_CAM_Sketches/blindspot_cam.ino for the sketch.
#
#  If static IPs are not set, the scanner tries mDNS hostnames:
#    blindspot-left.local, blindspot-right.local, blindspot-rear.local
#
#  As a last resort, nmap scans the subnet and queries /id.
# ──────────────────────────────────────────────────────────────

CAMERA_PORTS = {
    "left": {
        "port":  "/dev/ttyAMA0",
        "url":   "http://10.92.111.188/stream",   # Static IP (recommended)
        "baud":  115200,
        "label": "Left Camera",
    },
    "right": {
        "port":  "/dev/ttyAMA4",
        "url":   "http://10.92.111.190/stream",
        "baud":  115200,
        "label": "Right Camera",
    },
    "rear": {
        "port":  "/dev/ttyAMA2",
        "url":   "http://10.92.111.189/stream",
        "baud":  115200,
        "label": "Rear Camera",
    },
}
...
# ──────────────────────────────────────────────────────────────
#  DETECTION ZONE THRESHOLDS  (centimetres)
# ──────────────────────────────────────────────────────────────

ZONE = {
    "safe":     300,   # > 300 cm  → Green  (all clear)
    "caution":  200,   # ≤ 200 cm  → Yellow (vehicle nearby)
    "critical": 100,   # ≤ 100 cm  → Red    (imminent collision)
}
...
ULTRASONIC = {
    ...
    "readings_per_avg":    5,         # Median-filter window (odd)
    "polling_interval_s":  0.06,      # ~16 Hz  (HC-SR04 min cycle = 60 ms)
...
CAMERA = {
    ...
    "stream_frame_timeout":    5.0,           # Reconnect if no frame for 5 s
    ...
    "stream_max_buffer":  1_048_576,          # 1 MB max buffer before reset
...
WARNING = {
    ...
    "motor_duty_caution":   0,     # PWM duty cycle % for caution (0 = off)
    "motor_duty_critical": 80,     # PWM duty cycle % for critical vibration
